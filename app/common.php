...
/**
 * 获取客户端浏览器类型
 * @param null $bs 待检测的浏览器类型
 * @return bool|string|null
 */
function get_client_browser($bs = null) {
...
/**
 * 获取客户端操作系统类型
 * @param string $os 待检测的操作系统类型
 * @return bool|string|null
 */
function get_client_os($os = '') {
    if (isset($_SERVER["HTTP_USER_AGENT"])) {
        $user_agent = strtolower($_SERVER["HTTP_USER_AGENT"]);
    } else {
        return null;
    ...
    // 直接检测传递的值
    if (!empty($os)) {
        if (strpos($user_agent, strtolower($os))) {
            return true;
        } else {
            return false;
        }
 ...
 * @param array $fields 发送的参数
 * @param null $UserAgent 发送user-agent
 * @param bool $vfSSL 是否https请求
 * @return bool|string
 */
function curl_url($url, $fields = array(), $UserAgent = null, $vfSSL = false) {
...
/**
 * 服务器信息
 * @param bool $sensitive 敏感信息
 * @return mixed
 */
function get_server_info($sensitive = true) {
    define('YES', 'Yes');
    define('NO', '<span style="color:red">No</span>');
    // 服务器系统
    $data['php_os'] = $sensitive ? PHP_OS : NO;
    // 服务器访问地址
    ...
    // 站点目录
    $data['document_root'] = $sensitive ? isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : DOC_PATH : NO;
    // PHP版本
    $data['php_version'] = PHP_VERSION;
    // php配置文件
    $data['php_ini'] = $sensitive ? @php_ini_loaded_file() : NO;
    // 最大上传
    ...
    // 会话保存路径
    $data['session_save_path'] = $sensitive ? session_save_path() ?: $_SERVER['TMP'] : NO;
    // 检测standard库是否存在
...
function get_db_type() {
    switch (\think\facade\Config::get('database.default')) {
        case 'mysqli':
        case 'pdo_mysql':
            $db = 'mysql';
            break;
